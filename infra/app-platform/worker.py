...
def prepare_signer(directory):
    ...
    data = json.loads(raw)
    if data.get('address', '').lower().removeprefix('0x') != service.GOVERNOR[2:].lower():
    ...
    key = Path(directory) / 'robinhood-deployer'
    key.write_bytes(raw)
    key.chmod(0o600)
...
class CloudBackend(service.SignedBackend):
    def submit(self, transaction, nonce):
        expected = self.transaction(self.position_id)
        if (transaction['from'].lower() != service.GOVERNOR.lower()
        ...
        result = subprocess.run(['cast', 'send', '--rpc-url', self.url,
                                 ...
                                 '--from', service.GOVERNOR, '--nonce', str(nonce),
